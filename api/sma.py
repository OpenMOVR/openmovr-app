...
Provides SMA clinical summary data from either live parquet files or pre-computed snapshots.
...
class SMAAPI:
    """
    API for SMA clinical summary data.
    ...
    _snapshot_path: Path = Path(__file__).parent.parent / "stats" / "sma_snapshot.json"
    ...
    def _load_snapshot(cls) -> bool:
        ...
        possible_paths = [
            cls._snapshot_path,
            Path(__file__).parent.parent / "stats" / "sma_snapshot.json",
            Path("stats/sma_snapshot.json"),
        ]
        ...
        for path in possible_paths:
            if path.exists():
                try:
                    ...
                    print(f"Error loading SMA snapshot from {path}: {e}")
    ...
    @classmethod
    def get_classification(cls) -> Dict[str, Any]:
        """Get SMA type classification, diagnosis method, genetic confirmation."""
        snap = cls.get_snapshot()
        if not snap:
            return {}
        return snap.get("classification", {})

    @classmethod
    def get_genetics(cls) -> Dict[str, Any]:
        """Get genetics data (SMN1/SMN2 copy number, family history)."""
        snap = cls.get_snapshot()
        if not snap:
            return {}
    ...
    @classmethod
    def get_motor_scores(cls) -> Dict[str, Any]:
        """Get motor function scores (HFMSE, CHOP-INTEND, RULM)."""
        snap = cls.get_snapshot()
        if not snap:
            return {}
        return snap.get("motor_scores", {})

    @classmethod
    def get_therapeutics(cls) -> Dict[str, Any]:
        """Get SMA therapeutics (Spinraza, Evrysdi, Zolgensma)."""
        snap = cls.get_snapshot()
        if not snap:
            return {}
    ...
    def get_respiratory(cls) -> Dict[str, Any]:
        """Get respiratory function (FVC % predicted)."""
        snap = cls.get_snapshot()
        if not snap:
            return {}
    ...
    def get_milestones(cls) -> Dict[str, Any]:
        """Get disease milestones (ambulatory status)."""
        snap = cls.get_snapshot()
        if not snap:
            return {}
    ...
    def get_demographics(cls) -> Dict[str, Any]:
        """Get demographics (diagnosis age, enrollment age, gender)."""
        snap = cls.get_snapshot()
        if not snap:
            return {}
